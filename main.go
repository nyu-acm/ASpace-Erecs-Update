...
func main() {
	flag.Parse()
	domains = []string{"aeon.library.nyu.edu", "rstar.dlib.nyu.edu"}
	logfilename := fmt.Sprintf("erecs-update-%s-repository-%d.log", environment, repository)
	...
	log.SetOutput(f)
	log.Println("INFO", "erecs update tool")
	...
	for _, doId := range doIds {
		...
		for i,fv := range do.FileVersions {
			...
			if contains(u.Host) && (fv.UseStatement == "" || fv.UseStatement == "electronic-records" || fv.UseStatement == "electronic-records-service") {
				update = true
				fileversions[i].UseStatement = "electronic-records-reading-room"
			}
		}
